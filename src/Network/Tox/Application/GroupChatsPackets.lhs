...
All packet fields are considred mandatory unless flagged as \textbf{\verb'[optional]'}.
...
and 8 bytes of empty padding, which is used to mitigate certain types of
cryptography attacks. The \textbf{\verb'Group Packet Identifier'} is used to
identify the type of group packet, and the \textbf{\verb'Message ID'} is a
...
The public encryption key must be a brand new, unused key, which takes
